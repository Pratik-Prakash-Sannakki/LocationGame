...
function GeoLocation(props) {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      Promise.all([fetchUserLocation('user2'), fetchUserLocation('user3')])
        .then(locations => {
          const userLocations = locations.reduce((acc, loc) => ({ ...acc, ...loc }), {});
  ...
  return (
    <div style={{ backgroundColor: 'white', padding: 72 }}>
      ...
      <h4>User2</h4>
      {users.user2.latitude !== undefined && <p>Latitude: {users.user2.latitude}</p>}
      {users.user2.longitude !== undefined && <p>Longitude: {users.user2.longitude}</p>}
      <h4>User3</h4>
      {users.user3.latitude !== undefined && <p>Latitude: {users.user3.latitude}</p>}
      {users.user3.longitude !== undefined && <p>Longitude: {users.user3.longitude}</p>}
      <h1>Map</h1>
      {lat && lng && users.user3.latitude && users.user3.longitude && users.user2.latitude && users.user2.longitude && (
        <Map height={300} defaultCenter={[lat, lng]} defaultZoom={12} center={[lat, lng]}>
          <Marker width={50} anchor={[lat, lng]} />
          <Marker width={50} anchor={[Number(users.user2.latitude), Number(users.user2.longitude)]} />
          <Marker width={50} anchor={[Number(users.user3.latitude), Number(users.user3.longitude)]} />
        </Map>
      )}
    </div>
  );
